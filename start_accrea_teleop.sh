...
lerobot-teleoperate \
  ...
  --robot.max_delta_per_step_rad=0.1 \
  --robot.require_user_confirmation=false \
  --robot.cameras='{
    camera1:   {index_or_path: "/dev/video0", width: 640, height: 480, fps: 30},
    camera2: {index_or_path: "/dev/video2", width: 640, height: 480, fps: 30}
  }' \
  --teleop.type=accrea_gamepad_cartesian \
  --teleop.id=xbox_0 \
  --teleop.hz=60 \
  --fps=30 \
  --teleop.lin_vel_mps=1.4 \
  --teleop.roll_vel_rps=1.4 \
  --teleop.pitch_vel_rps=1.4 \
  --teleop.yaw_vel_rps=1.4 \
  --teleop.damping_lambda=0.04 \
  --teleop.max_qd_rad_s=2.2 \
  --teleop.ee_link=tcp \
  --display_data=true \
